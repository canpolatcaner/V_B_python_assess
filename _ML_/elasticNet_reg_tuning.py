...
enet_cv_model = ElasticNet().fit(X_train, y_train)
...
# Tahmin yapalım
X_train_tahmin = enet_cv_model.predict(X_train)[0:10]
print("\n\nX_train_tahmin:",X_train_tahmin)

X_test_tahmin = enet_cv_model.predict(X_test)[0:10]
print("\n\nX_test_tahmin:",X_test_tahmin)

# Test hatalarına bakalım.
y_pred = enet_cv_model.predict(X_test) # Tahmin edilen değerler
...
enet_cv_model= ElasticNetCV(alphas=alfalar,cv=10).fit(X_train, y_train)
